...
for ins_number in range(32,33):
    ...
    opt.add(cumulative(y_coord,circuit_y , circuit_x, width))
    opt.add(cumulative(x_coord, circuit_x , circuit_y, h_to_min))
    ...
    for i in range(1, n_circuits):
        for j in range(0, i):
            opt.add(Or (x_coord[i] + circuit_x[i] <= x_coord[j],
                                x_coord[j] + circuit_x[j] <= x_coord[i],
                                y_coord[i] + circuit_y[i] <= y_coord[j],
                            y_coord[j] + circuit_y[j] <= y_coord[i])
                        )
    ...
    for i in range(1,n_circuits):
        for j in range(1,n_circuits):
            if (circuit_x[i] == circuit_x[j]) and (circuit_y[i] + circuit_y[j] == h_to_min) and (i != j) and  (n_circuits != 8) and (n_circuits != 10):
    ...
    if opt.check() == sat:
        ...
        for i in range(n_circuits):
            out += "{} {} {} {}\n".format(circuit_x[i], circuit_y[i], model.evaluate(x_coord[i]), model.evaluate(y_coord[i]))
